...
class _PaywallScreenState extends ConsumerState<PaywallScreen> {
  ...
  static const String _privacyPolicyUrl = 'https://youh4ck3dme.github.io/cv-app/privacy';
  static const String _termsOfUseUrl = 'https://youh4ck3dme.github.io/cv-app/terms';
  ...
  Future<void> _handlePurchase() async {
    ...
    try {
      ...
      if (offerings.current != null && offerings.current!.availablePackages.isNotEmpty) {
        ...
        // Check if the entitlement is active after purchase
        if (customerInfo.entitlements.all['premium']?.isActive ?? false) {
          if (mounted) {
            Navigator.of(context).pushReplacement(
              MaterialPageRoute(builder: (context) => const PreviewScreen(isUnlocked: true)),
            );
          }
        } else {
          _showErrorSnackBar("Premium entitlement is inactive. Please try restoring purchases or contact support.");
        }
      } else {
      ...
      if (errorCode == PurchasesErrorCode.purchaseCancelledError) {
        ...
        _showErrorSnackBar("Payment is pending. Your premium status will update once completed.");
      } else {
        _showErrorSnackBar("Purchase failed: ${e.message ?? 'Unknown error'}");
      }
    } catch (e) {
      _showErrorSnackBar("Could not complete the transaction. Please try again.");
    } finally {
  ...
  Future<void> _restorePurchases() async {
    ...
    try {
      ...
      if (restoredInfo.entitlements.all['premium']?.isActive ?? false) {
        _showSuccessSnackBar("Purchases successfully restored!");
        if (mounted) {
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(builder: (context) => const PreviewScreen(isUnlocked: true)),
          );
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: SizedBox(
                          ...
                          child: ElevatedButton(
                            ...
                            .shimmer(duration: 2.seconds, color: Colors.white.withAlpha(128)),
                      )
                          .animate()
                      ...
                      const Text(
                        ...
                        style: TextStyle(
                          ...
                          fontSize: 14,
                          fontWeight: FontWeight.w600, // Semi-bold for high visibility
                        ),
                      )
                          .animate()
                          .fade(delay: 750.ms, duration: 500.ms),
                      ...
                      Padding(
                        ...
                        child: Wrap(
                          ...
                          children: [
                            ...
                            _divider(),
                            _ComplianceButton(
                              text: "Terms of Service", // Google Play compliance
                              onPressed: (_isPurchasing || _isRestoring) ? null : () => _launchURL(_termsOfUseUrl),
                            ),
                            _divider(),
                            _ComplianceButton(
                              text: "Privacy Policy",
                              onPressed: (_isPurchasing || _isRestoring) ? null : () => _launchURL(_privacyPolicyUrl),
                            ),
                          ],
                        ),
                      ),
            ...
            child: SafeArea(
              child: Container(
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: Colors.white.withAlpha(30), // subtle white border for premium design
                    width: 1,
                  ),
                ),
                child: IconButton(
  ...
  Widget _divider() => const Text("  |  ", style: TextStyle(color: Colors.grey, fontSize: 10));
}
...
class DummyPdfPreviewPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<DummyPdfPreviewPage> createState() => _DummyPdfPreviewPageState();
}
...
class _DummyPdfPreviewPageState extends ConsumerState<DummyPdfPreviewPage> {
  ...
  Future<void> _loadPdfPreview() async {
    try {
      ...
      if (currentCv != null) {
        final pdfBytes = await PdfService.generatePdf(currentCv);
        
        // Rasterize the first page of the generated PDF using printing package
        await for (final page in Printing.raster(pdfBytes, pages: [0], dpi: 100)) {
